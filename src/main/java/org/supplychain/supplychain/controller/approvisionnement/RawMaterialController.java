...
public class RawMaterialController {
    ...
    public ResponseEntity<SuccessResponse<MaterialResponse>> create(@Valid @RequestBody MaterialRequest request, HttpServletRequest req) {
        MaterialResponse res = rawMaterialService.createRawMaterial(request);
        return ResponseEntity.status(HttpStatus.CREATED).body(SuccessResponse.of(HttpStatus.CREATED, "Matière créée", res, req.getRequestURI()));
    }
    @PutMapping("/{id}")
    @PreAuthorize("hasRole('GESTIONNAIRE_APPROVISIONNEMENT')")
    public ResponseEntity<SuccessResponse<MaterialResponse>> update(@PathVariable Long id, @Valid @RequestBody MaterialRequest request, HttpServletRequest req) {
